...
def show_cell(cell_idx, first_frame=0, last_frame=50, frame_size=150):
    ...
    with h5py.File(hdf5_file, 'r') as f:
        xcentre = np.array([f['Features'][f'Cell{cellidx:04}']['Morphological Features'][int(framei)]['xcentre'] for framei in
                            range(first_frame, last_frame)])
        ycentre = np.array([f['Features'][f'Cell{cellidx:04}']['Morphological Features'][int(framei)]['ycentre'] for framei in
                            range(first_frame, last_frame)])
