...
class CellBatch:
    def __init__(self, indices):
        ...
        self.masks = torch.where(full_mask.unsqueeze(0).expand(len(indices), *full_mask.shape) == mask_indices, 1, 0)

    def run_feature_extraction(self):
        for path in sorted(SETTINGS.DIRECTORY / 'tracked' / 'phase').iterdir():
